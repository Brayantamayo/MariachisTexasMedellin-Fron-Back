...
// ✅ FIX: obtener fecha local en formato YYYY-MM-DD sin conversión UTC
// new Date().toISOString() convierte a UTC restando 5h en Colombia
...
export const RehearsalCreateModal: React.FC<Props> = ({ isOpen, onClose, onSave }) => {
  const emptyRehearsal = {
    title:         '',
    location:      '',
    date:          getTodayLocal(), // ✅ hora local, no UTC
    time:          '',
    notes:         '',
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-5xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh] animate-fade-in-up overflow-hidden">
        ...
        <div className="px-8 py-6 border-t border-slate-100 bg-white flex justify-end gap-4 z-10">
          <button onClick={onClose} className="text-xs font-bold text-slate-400 hover:text-slate-600 transition-colors uppercase tracking-widest px-4 py-2">
            Cancelar
          </button>
          <button
