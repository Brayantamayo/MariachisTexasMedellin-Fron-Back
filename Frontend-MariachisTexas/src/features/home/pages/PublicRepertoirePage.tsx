...
import { Play, Pause, Clock, Search, Music, Heart, Disc, Mic2, ListFilter, Sparkles, Volume2, SkipBack, SkipForward, VolumeX, X } from 'lucide-react';
import { repertoireService } from '../../repertoire/services/repertoireService';
import { Song } from '@/types';
import { motion, AnimatePresence } from "motion/react";
import toast from 'react-hot-toast';

// Simple Equalizer Component
const Equalizer = () => (
...
export const PublicRepertoirePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSongs = async () => {
      try {
        ...
        setSongs(data.filter(s => s.isActive));
      } catch (error) {
  ...
  }, [songs, currentSong]); // Re-bind if songs change (for playNext closure if needed, though we use state)
                ...
                >
                    <div className="w-8 text-center text-white/50 font-medium relative flex items-center justify-center min-h-[20px]">
                        {currentSong?.id === song.id && isPlaying ? (
                            ...
                            <span className={`group-hover:hidden text-sm ${currentSong?.id === song.id ? 'text-[#1ed760]' : ''}`}>{index + 1}</span>
                        )}
                    </div>
                    ...
                    <div className="w-12 text-center text-sm text-white/50 font-variant-numeric tabular-nums flex items-center justify-center gap-4">
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
                                setLyricsSong(song);
                            }}
                            className={`hidden group-hover:block hover:text-[#1ed760] transition-colors ${hoveredRow === song.id ? 'opacity-100' : 'opacity-0'}`}
                            title="Ver Letra"
                        >
                            <Mic2 size={16} />
                        </button>
                        <Heart size={16} className={`hidden group-hover:block hover:text-[#1ed760] hover:fill-[#1ed760] transition-colors ${hoveredRow === song.id ? 'opacity-100' : 'opacity-0'}`} />
                        {song.duration}
                    </div>
                </motion.div>
            ))}
            ...
            >
                ...
                <div className="flex items-center gap-4 w-full md:w-1/3 min-w-0">
                    ...
                    <div className="flex flex-col min-w-0">
                        ...
                        <span className="text-white/60 text-xs hover:underline cursor-pointer truncate">{currentSong.artist}</span>
                    </div>
                    <button onClick={closePlayer} className="text-white/50 hover:text-white cursor-pointer ml-4 hidden sm:block" title="Cerrar">
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="flex flex-col items-center gap-2 w-full md:w-1/3">
                    ...
                    <div className="w-full flex items-center gap-2 text-xs text-white/50 font-variant-numeric tabular-nums">
