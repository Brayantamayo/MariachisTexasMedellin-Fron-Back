...
import { FileText, ArrowLeft, CheckCircle, Music, Star } from 'lucide-react';
...
export const PublicCotizacionPage: React.FC<Props> = ({ onNavigate }) => {
  ...
  const isAdmin = false; // Always false for public page
  ...
  // Estado para controlar si el precio fue editado manualmente (no aplica aquí realmente, pero requerido por lógica)
  ...
  // Lógica de Cálculo de Precio Automático (Idéntica a CotizacionCreateModal)
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (blockStatus.isBlocked) {
        toast.error(`La fecha está bloqueada: ${blockStatus.reason}`);
        return;
    }

    if (!formData.startTime || !formData.endTime) {
        toast.error("Por favor selecciona la hora de inicio y fin.");
        return;
    }
    ...
    if (!hasBaseService) {
        toast.error("Por favor selecciona un Tipo de Serenata.");
        return;
    }
      ...
      return (
          <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4 pt-24 relative overflow-hidden">
              ...
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 max-w-md w-full p-8 rounded-3xl shadow-2xl text-center relative z-10">
                  <div className="w-20 h-20 bg-gradient-to-br from-green-400 to-emerald-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg shadow-green-900/20">
                      <CheckCircle size={40} className="text-white" />
                  </div>
                  <h2 className="text-3xl font-serif font-bold text-white mb-3">¡Cotización Recibida!</h2>
                  <p className="text-slate-300 mb-8 leading-relaxed">
                      Hemos recibido tu solicitud correctamente. Nos pondremos en contacto contigo a la brevedad posible al número <strong className="text-orange-400">{formData.clientPhone}</strong> o al correo <strong className="text-orange-400">{formData.clientEmail}</strong>.
                  </p>
                  <div className="flex gap-4 justify-center">
                      ...
                      <button 
                          ...
                          className="px-6 py-3 bg-gradient-to-r from-orange-500 to-red-600 text-white font-bold rounded-xl hover:from-orange-400 hover:to-red-500 transition-all shadow-lg shadow-orange-900/20"
                      >
                          Nueva Cotización
                      </button>
                  </div>
  ...
  return (
    <div className="min-h-screen bg-[#050505] pb-24 pt-32 px-4 font-sans relative selection:bg-orange-500/30">
      {/* Ambient Background */}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="text-center mb-16 relative">
             <div className="inline-flex items-center justify-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-sm mb-6">
                <Star size={12} className="text-orange-400 fill-orange-400" />
                <span className="text-orange-200 font-bold tracking-[0.2em] text-[10px] uppercase">
                    Planea tu Evento Perfecto
                </span>
                <Star size={12} className="text-orange-400 fill-orange-400" />
             </div>
             
             <h1 className="text-5xl md:text-6xl lg:text-7xl font-serif font-medium tracking-tight text-white mb-6 drop-shadow-2xl">
                Solicitar <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-200 via-orange-400 to-orange-200">Cotización</span>
             </h1>
             
             <p className="text-slate-400 max-w-2xl mx-auto text-base md:text-lg leading-relaxed font-light">
                Personaliza tu experiencia con <span className="text-white font-medium">Mariachis Texas</span>. 
                Selecciona tu repertorio, fecha y servicios adicionales para obtener un presupuesto inmediato.
             </p>
          </div>

          {/* Form Container */}
          <div className="bg-white rounded-[2rem] shadow-2xl shadow-black/50 overflow-hidden border border-white/10 relative backdrop-blur-sm">
