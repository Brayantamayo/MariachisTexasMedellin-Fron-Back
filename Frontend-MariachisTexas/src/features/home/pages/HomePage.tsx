...
export const HomePage: React.FC = () => {
  ...
  // --- VISTA DE CLIENTE (SIMPLIFICADA Y ELEGANTE) ---
  return (
        ...
        <div className="relative rounded-[2rem] overflow-hidden shadow-2xl min-h-[400px] flex items-center justify-center group">
            ...
            <div className="relative z-10 w-full max-w-6xl mx-auto px-6 md:px-12 py-12">
                ...
                {clientEvents.length > 0 ? (
                    // LAYOUT CON EVENTOS: Dos columnas equilibradas
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                        <div className="space-y-6 text-left">
                            ...
                            <div className="flex flex-wrap gap-4 pt-2">
                                <button className="bg-[#ce1126] hover:bg-[#b91c1c] text-white px-8 py-3.5 rounded-full font-bold text-sm uppercase tracking-widest transition-all shadow-lg hover:shadow-red-900/40 hover:-translate-y-1 flex items-center gap-2">
                                    <Star size={18} className="fill-white" /> Nuevo Evento
                                </button>
                                <button className="bg-white/10 hover:bg-white/20 text-white border border-white/20 px-8 py-3.5 rounded-full font-bold text-sm uppercase tracking-widest transition-all backdrop-blur-sm">
                                    Ver Repertorio
                                </button>
                            </div>
                        </div>
                            ...
                            <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-[2rem] p-8 text-white shadow-2xl relative overflow-hidden">
                                ...
                                <div className="space-y-3 text-sm text-slate-200">
                                    <p className="flex items-center gap-3 bg-black/20 p-3 rounded-xl border border-white/5">
                                        <Clock size={16} className="text-[#f1bf00]" /> 
                                        <span className="font-medium">{clientEvents[0].eventTime}</span>
                                    </p>
                                    <p className="flex items-center gap-3 bg-black/20 p-3 rounded-xl border border-white/5">
                                        <MapPin size={16} className="text-[#ce1126]" /> 
                                        <span className="truncate font-medium">{clientEvents[0].location}</span>
                                    </p>
                                </div>
                                ...
                                <div className="mt-6 pt-6 border-t border-white/10 flex justify-between items-center">
                                    ...
                                    <button className="text-sm font-bold text-white hover:underline">Ver Detalles</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ) : (
                    // LAYOUT SIN EVENTOS: Centrado y Limpio
                    <div className="flex flex-col items-center text-center max-w-2xl mx-auto space-y-8">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 w-full justify-center pt-4">
                            <button className="bg-[#ce1126] hover:bg-[#b91c1c] text-white px-10 py-4 rounded-full font-bold text-sm uppercase tracking-widest transition-all shadow-xl shadow-red-900/30 hover:-translate-y-1 hover:shadow-2xl flex items-center justify-center gap-3 min-w-[200px]">
                                <Star size={18} className="fill-white" />
                                Agendar Evento
                            </button>
                            <button className="bg-white/5 hover:bg-white/10 text-white border border-white/10 px-10 py-4 rounded-full font-bold text-sm uppercase tracking-widest transition-all backdrop-blur-sm flex items-center justify-center gap-3 min-w-[200px]">
                                <ListMusic size={18} />
                                Ver Repertorio
                            </button>
                        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
                <div className="flex items-center justify-between">
                    <h3 className="font-bold text-slate-800 text-xl flex items-center gap-3">
                        <span className="w-8 h-8 rounded-lg bg-[#ce1126]/10 flex items-center justify-center text-[#ce1126]">
                            <Calendar size={18} />
                        </span>
                        Mis Reservas
                    </h3>
                    <button className="text-xs font-bold text-slate-400 hover:text-[#ce1126] uppercase tracking-wider transition-colors">
                        Ver Historial
                    </button>
                </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-sm text-slate-500">
                                            <p className="flex items-center gap-1.5">
                                                <Clock size={14} className="text-[#f1bf00]" /> 
                                                {event.eventTime}
                                            </p>
                                ...
                                <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto border-t sm:border-t-0 border-slate-100 pt-4 sm:pt-0">
                                    <div className="text-right">
                                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider mb-1 ${
                                            ...
                                            event.status === 'Pendiente' ? 'bg-amber-100 text-amber-700 border border-amber-200' : 'bg-slate-100 text-slate-600 border border-slate-200'
                                        }`}>
                                            {event.status === 'Confirmado' && <CheckCircle size={10} />}
                                            {event.status}
                                        </span>
                                    ...
                                    <button className="w-10 h-10 rounded-full bg-slate-50 hover:bg-slate-100 text-slate-400 hover:text-slate-700 flex items-center justify-center transition-colors">
            ...
            <div className="space-y-6">
                ...
                <div className="bg-gradient-to-br from-[#1e293b] to-[#0f172a] p-8 rounded-[2.5rem] shadow-xl text-white relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-sm text-slate-400 mb-8 leading-relaxed">
                            Estamos aquí para resolver tus dudas sobre eventos, pagos o repertorio.
                        </p>
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors cursor-pointer group/item">
                                ...
                                <div>
                                    ...
                                    <p className="font-bold text-sm">+57 300 123 4567</p>
                                </div>
                            </div>
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button className="flex flex-col items-center justify-center p-4 rounded-2xl bg-slate-50 hover:bg-[#ce1126]/5 hover:text-[#ce1126] border border-transparent hover:border-[#ce1126]/10 transition-all gap-2 group">
                        ...
                        <button className="flex flex-col items-center justify-center p-4 rounded-2xl bg-slate-50 hover:bg-[#f1bf00]/10 hover:text-[#b45309] border border-transparent hover:border-[#f1bf00]/20 transition-all gap-2 group">
                            <Star size={24} className="text-slate-400 group-hover:text-[#f1bf00] transition-colors" />
                            <span className="text-[10px] font-bold uppercase tracking-wide">Calificar</span>
                        </button>
                    </div>
                </div>
            </div>
