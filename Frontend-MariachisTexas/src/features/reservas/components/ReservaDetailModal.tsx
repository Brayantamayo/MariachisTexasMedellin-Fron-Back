...
export const ReservaDetailModal: React.FC<Props> = ({ isOpen, onClose, reservation, onFinalize }) => {
  ...
  useEffect(() => {
      if (isOpen) {
          ...
          // ✅ Cargar servicios para mostrar nombre y precio en el detalle
  ...
  // ✅ Construir lista de servicios contratados con nombre y precio real
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-4 items-center bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <div className="flex-1 min-w-[200px]">
                      <p className="font-bold text-slate-800 text-sm">{reservation.location}</p>
                            ...
                            <div key={service.id} className="flex items-center justify-between p-3 rounded-xl border border-slate-100 bg-slate-50/50">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-slate-400">${service.precio.toLocaleString()} c/u</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-xs font-bold text-slate-500">x{service.quantity}</p>
                                    <p className="text-sm font-bold text-orange-600">
                                        ${(service.precio * service.quantity).toLocaleString()}
                                    </p>
                                </div>
                            </div>
                        ))}
