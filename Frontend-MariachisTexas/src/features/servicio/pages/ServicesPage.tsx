...
import { Plus, Search, Briefcase, CheckCircle, AlertCircle, X } from 'lucide-react';
...
import { ServiceEditModal } from '@/src/features/servicio/components/ServiceEditModal.tsx';
...
export const ServicesPage: React.FC = () => {
  ...
  const handleCreate = async (serviceData: Omit<Service, 'id' | 'isActive'>) => {
    try {
  ...
  const handleUpdate = async (serviceData: Omit<Service, 'id' | 'isActive'>) => {
  ...
  const handleToggleStatus = async (service: Service) => {
    ...
    try {
      ...
      await servicesService.updateService(service.id, { isActive: newStatus });
    } catch (error) {
      console.error(error);
      fetchServices();
