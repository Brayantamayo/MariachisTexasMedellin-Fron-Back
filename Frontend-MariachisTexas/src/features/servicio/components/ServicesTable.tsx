...
import { Edit2, Trash2, Eye, DollarSign, Briefcase } from 'lucide-react';
...
export const ServicesTable: React.FC<Props> = ({ 
  ...
  if (loading) {
    return <div className="text-center py-20 text-slate-400">Cargando servicios...</div>;
  }

  if (services.length === 0) {
    return <div className="text-center py-20 text-slate-400">No hay servicios registrados.</div>;
  }
                    ...
                    <div className="flex items-center justify-center gap-2">
                        {canManage ? (
                            <button 
                                ...
                                className={`relative w-9 h-5 rounded-full transition-all duration-300 focus:outline-none ${service.isActive ? 'bg-emerald-500' : 'bg-slate-200'}`}
                            >
                                <span 
                                    className={`absolute top-0.5 left-0.5 bg-white w-4 h-4 rounded-full shadow-sm transition-transform duration-300 ${service.isActive ? 'translate-x-4' : 'translate-x-0'}`} 
                                />
                            </button>
                        ) : (
                            <div className={`w-2 h-2 rounded-full ${service.isActive ? 'bg-emerald-500 shadow-sm' : 'bg-slate-300'}`}></div>
                        )}
                        <span className={`text-[10px] font-bold uppercase tracking-wider w-10 ${service.isActive ? 'text-emerald-500' : 'text-slate-300'}`}>
