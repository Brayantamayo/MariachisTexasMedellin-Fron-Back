...
import { User, Calendar, MapPin, Map, Search, ChevronDown, DollarSign, ShieldAlert, AlertTriangle, Calculator, Plus, Minus, Package, Music, X, Check, ArrowLeft } from 'lucide-react';
...
export const CotizacionForm: React.FC<Props> = ({ 
  ...
  const baseServices = services.filter(s => s.name.toLowerCase().includes('serenata'));
  const additionalServices = services.filter(s => !s.name.toLowerCase().includes('serenata'));
  ...
  const handleBaseServiceSelect = (serviceId: string) => {
      ...
      if (!isCurrentlySelected) {
          baseServices.forEach(bs => {
              if (bs.id !== serviceId) {
                  const selected = formData.selectedServices?.find((s: any) => s.serviceId === bs.id);
  ...
  // Derive extra quantities from services
  const extraHoursService = services.find(s => s.name === 'Hora Extra');
  const extraSongsService = services.find(s => s.name === 'Canción Extra');
  ...
  const extraSongsQuantity = extraSongsService
      ? (formData.selectedServices?.find((s: any) => s.serviceId === extraSongsService.id)?.quantity || 0)
      : 0;
  ...
  // Update endTime when startTime or extraHours changes
  React.useEffect(() => {
      if (formData.startTime) {
          const newEndTime = calculateEndTime(formData.startTime, extraHoursQuantity);
          if (formData.endTime !== newEndTime) {
              // Simulate event for onChange to update parent state
              onChange({
                  target: { name: 'endTime', value: newEndTime }
              } as any);
          }
      }
  }, [formData.startTime, extraHoursQuantity, onChange, formData.endTime]);
  ...
  // Dynamic styles based on isPublic
  ...
  return (
    <form id="cotizacion-form" onSubmit={onSubmit} className="flex flex-col lg:flex-row h-full">
        ...
        <div className={`w-full lg:w-7/12 p-8 lg:p-10 space-y-10 ${isPublic ? 'bg-white' : 'bg-white border-r border-slate-100'}`}>
            ...
            <div className={isPublic ? "" : "bg-white p-5 rounded-xl border border-slate-100 shadow-sm"}>
                                ...
                                <div className="relative">
                                    ...
                                    >
                                        ...
                                        {formData.startTime && !availableHours.includes(formData.startTime) && (
                                            <option key={`start-current-${formData.startTime}`} value={formData.startTime}>{formData.startTime} (Actual)</option>
                                        )}
                                    </select>
                ...
                <div className="mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {baseServices.map(service => {
                            const isSelected = formData.selectedServices?.find((s: any) => s.serviceId === service.id)?.quantity > 0;
                            return (
                                <div 
                                    key={service.id}
                                ...
                                >
                                    ...
                                    <h5 className={`font-bold text-lg mb-1 ${isSelected ? 'text-orange-800' : 'text-slate-700'}`}>{service.name}</h5>
                                    <p className="text-sm text-slate-500 mb-3">{service.description}</p>
                                    <p className={`font-bold ${isSelected ? 'text-orange-600' : 'text-slate-600'}`}>${service.price.toLocaleString()}</p>
                                </div>
                            );
                        })}
                    </div>
                ...
                <div>
                    ...
                    <textarea name="repertoireNotes" value={formData.repertoireNotes} onChange={onChange} className={`${inputClass} min-h-[100px] resize-none`} placeholder="¿Alguna petición especial, dedicatoria o indicación para llegar?" />
                </div>
            </div>
        </div>
        ...
        <div className={`w-full lg:w-5/12 p-0 flex flex-col ${isPublic ? 'bg-slate-50 border-l border-slate-100' : 'bg-slate-50'}`}>
            ...
            {/* Lista Servicios y Repertorio */}
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                
                {/* 1. Buscador de Repertorio */}
                <div className={`p-8 border-b border-slate-100 ${isPublic ? 'bg-white' : 'bg-white'}`}>
                    <div className={`flex items-center justify-between mb-6 border-b pb-4 ${isPublic ? 'border-orange-100/50' : 'border-primary-100'}`}>
                        <h4 className={headerClass.replace('mb-8', 'mb-0').replace('border-b', '').replace('pb-4', '')}>
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isPublic ? 'bg-orange-100 text-orange-600' : 'text-primary-600'}`}>
                ...
                <div className={`p-8 border-b border-slate-100 ${isPublic ? 'bg-slate-50/50' : 'bg-slate-50/30'}`}>
                    <h4 className={headerClass.replace('mb-8', 'mb-6').replace('border-b', '').replace('pb-4', '')}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isPublic ? 'bg-orange-100 text-orange-600' : 'text-primary-600'}`}>
                    ...
                    <div className="space-y-4">
                        {additionalServices.map(service => {
                            const selected = formData.selectedServices?.find((s: any) => s.serviceId === service.id);
                            ...
                            return (
                                <div key={service.id} className={`flex items-center justify-between p-5 rounded-2xl border transition-all duration-300 ${quantity > 0 ? 'bg-white border-orange-200 shadow-lg shadow-orange-900/5 ring-1 ring-orange-100' : 'bg-white border-slate-100 shadow-sm hover:border-slate-200'}`}>
                                    <div>
                                        <p className={`text-sm font-bold ${quantity > 0 ? 'text-slate-800' : 'text-slate-600'}`}>{service.name}</p>
                                        <p className="text-xs text-slate-400 mt-1 font-medium">${service.price.toLocaleString()}</p>
                                    </div>
            ...
            <div className={`p-8 border-t border-slate-200 ${isPublic ? 'bg-white shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] relative z-20' : 'bg-white'}`}>
                <div className="space-y-3 mb-8 text-sm">
                    ...
                    {/* Mostrar desglose de servicios si hay */}
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="relative">
                            ...
                            <input 
                                type="text" 
                                name="totalAmount"
                                value={isAdmin ? (formData.totalAmount || 0).toLocaleString() : (formData.totalAmount || 0).toLocaleString()}
                                onChange={(e) => {
                                    if (!isAdmin) return;
                ...
                <div className="flex gap-4">
                    <button type="button" onClick={onCancel} className="flex-1 py-4 border border-slate-200 rounded-xl text-xs font-bold uppercase text-slate-500 hover:bg-slate-50 hover:text-slate-700 transition-colors">Cancelar</button>
                    <button 
                        type="submit" 
