...
import { X, Calendar, Clock, MapPin, User, Music, FileText, DollarSign, Mail, Phone, Hash, Tag, Map, Check, Package } from 'lucide-react';
                    ...
                    <div>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm space-y-4">
                            {quotation.homenajeado && (
                                <div className="grid grid-cols-1 gap-4 pb-4 border-b border-slate-50">
                                    <div>
                    ...
                    {quotation.selectedServices && quotation.selectedServices.length > 0 && (
                        <div className="p-6 border-b border-slate-100 bg-white">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-3">
                                <Package size={14} className="text-primary-600" /> Servicios Adicionales
                            </h4>
                            <div className="space-y-2">
                                {quotation.selectedServices.map(item => {
                                    ...
                                    return (
                                        <div key={item.serviceId} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-200">
                                            <div>
                                                <p className="text-sm font-bold text-slate-700">{service.name}</p>
                                                <p className="text-[10px] text-slate-400">${service.price.toLocaleString()} x {item.quantity}</p>
                                            </div>
                                            <p className="text-sm font-bold text-slate-800">
                                                ${(service.price * item.quantity).toLocaleString()}
                                            </p>
                                        </div>
                                    );
                                })}
