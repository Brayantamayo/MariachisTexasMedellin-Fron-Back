...
// permissions siempre contiene nombres de módulo: ["dashboard", "clientes", ...]
export const RoleForm: React.FC<Props> = ({
  formData, onChange, onTogglePermission, onSubmit, loadingPermissions,
}) => {
  return (
    <form id="role-form" onSubmit={onSubmit} className="space-y-8">
      ...
      <div className="space-y-5">
        <div>
          ...
          <input
            type="text" name="name" required value={formData.name} onChange={onChange}
        ...
        <div>
          ...
          <textarea
            name="description" rows={2} value={formData.description} onChange={onChange}
      ...
      <div>
        <div className="flex items-center justify-between mb-4 px-1">
          <h4 className="text-xs font-serif font-bold text-slate-700 uppercase tracking-widest">Acceso a Módulos</h4>
        ...
        {loadingPermissions ? (
          <div className="flex items-center justify-center py-10 text-slate-400 text-xs font-bold tracking-widest uppercase gap-2">
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-center mb-1">
                      <h5 className={`text-sm font-bold ${isAllowed ? 'text-slate-800' : 'text-slate-500'}`}>{module.label}</h5>
                      <div className={`w-10 h-5 rounded-full relative transition-colors duration-300 ${isAllowed ? 'bg-emerald-500' : 'bg-slate-300'}`}>
                        <div className={`absolute top-1 left-1 w-3 h-3 rounded-full bg-white shadow-sm transition-transform duration-300 ${isAllowed ? 'translate-x-5' : 'translate-x-0'}`} />
                      </div>
                    </div>
