...
export const Sidebar: React.FC<SidebarProps> = ({ currentPath, onNavigate, isPanelOpen, setIsPanelOpen }) => {
  ...
  const getCategories = (): Category[] => {
    ...
    if (isAdmin || isEmpleado) {
        serviciosItems.push({ label: 'Ensayos', path: '/ensayos', icon: Calendar });
    }

    if (isAdmin) {
        serviciosItems.push({ label: 'Servicios Extra', path: '/servicios', icon: Briefcase });
    }
    ...
    if (isAdmin) {
        ...
        ventaItems.push({ 
            label: 'Ventas', 
            path: '/ventas_parent', // Dummy path for parent
            icon: ShoppingCart,
            subItems: [
        ...
        // Empleado (Solo ve Reservas asignadas, por ejemplo)
        ventaItems.push({ label: 'Reservas', path: '/reservas', icon: Bookmark });
    }
  ...
  return (
    <div className="fixed left-0 top-0 h-screen flex z-50 shadow-2xl">
      ...
      <div className="w-24 bg-black flex flex-col items-center py-6 border-r border-white/5 z-20">
        ...
        {/* Logout - Moved to secondary panel */}
      </div>
      ...
      <div className={`h-screen bg-[#09090b] flex flex-col border-r border-white/5 z-10 relative transition-all duration-300 ease-in-out overflow-hidden ${isPanelOpen ? 'w-64 opacity-100' : 'w-0 opacity-0'}`}>
                ...
                return (
                    <div key={item.path} className="space-y-1">
                        <button
                            onClick={() => {
                                if (hasSubItems) {
                                    ...
                                    // Auto-hide panel on desktop for "work comfort" if desired, 
                                    // or just keep it for mobile. The user asked to "ocultar para trabajar mas comodo"
                                    // so let's auto-hide on navigation UNLESS pinned.
                                    if (!isPinned) {
                                        setIsPanelOpen(false);
                                    }
                                }
                        ...
                        >
                            <Icon 
                                size={18} 
                                className={`mr-3 transition-transform duration-300 ${isActive ? 'scale-110' : 'group-hover:scale-105'}`} 
                            />
        ...
        <div className="p-4 border-t border-white/5 bg-black/20 min-w-[16rem]">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5">
                ...
                <button
                    onClick={logout}
                    className="p-2 rounded-lg text-zinc-400 hover:text-red-500 hover:bg-white/5 transition-colors"
                    title="Cerrar Sesión"
                >
                    <LogOut size={18} />
                </button>
